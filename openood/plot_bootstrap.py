...
if args.postprocessor == 'salvim':
    ...
    occ_metric = get_metrics(
        f'saved_metrics/{args.dataset}_{args.postprocessor}_occlusion_range_bootstrapped.pkl'
    )
...
\\begin{{tabular}}{{ |c|c!{{\\vrule width 1pt}}c|c!{{\\vrule width 1pt}}c|c| }}
    ...
    print(f"""    \\hline
    \end{{tabular}}
    \caption[Wilcoxon signed-rank test for {args.postprocessor} on {pp(args.dataset)}]{{Results of performing a Wilcoxon signed-rank test on the \\ac{{auroc}} means of against \\ac{{mls}} and \\ac{{msp}}, showing the mean \\ac{{auroc}} over 10 runs on {pp(args.dataset)}, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \\texttt{{R}}-standard.}}
...
\\begin{{tabular}}{{ |c|c|c|c| }}
    ...
    print(f"""    \\hline
    \end{{tabular}}
    \caption[Wilcoxon signed-rank test for {args.postprocessor} on {pp(args.dataset)}]{{Results of performing a Wilcoxon signed-rank test on the \\ac{{auroc}} means of against \\ac{{vim}}, showing the mean \\ac{{auroc}} over 10 runs on {pp(args.dataset)}, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \\texttt{{R}}-standard.}}
    \label{{table:{args.dataset}_{args.postprocessor}_ttest}}
\end{{center}}
\setlength\\tabcolsep{{6pt}}
\end{{table}}""")
