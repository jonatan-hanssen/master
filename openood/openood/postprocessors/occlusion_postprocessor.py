...
class OcclusionVIMPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            for batch in tqdm(
                ...
                feature_id_train.append(numpify(feature))

                batch_saliencies = occlusion(net, data)
                saliencies.append(numpify(batch_saliencies))

            saliencies = np.concatenate(saliencies, axis=0)
            feature_id_train = np.concatenate(feature_id_train, axis=0)
            stacked_feature_id_train = np.hstack([feature_id_train, saliencies])

            logit_id_train = feature_id_train @ self.w.T + self.b

            self.u = -np.matmul(pinv(self.w), self.b)
            self.u = np.concatenate([self.u, np.zeros(saliencies.shape[1])])
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        saliencies = numpify(saliencies)

        stacked_feature_ood = np.hstack([feature_ood, saliencies])
