...
class LimeVIMPostprocessor(BasePostprocessor):
    ...
    def setup(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            for batch in tqdm(
                ...
                batch_betas = lime_explanation(net, data)
                betas.append(batch_betas.numpy())

            betas = np.concatenate(betas, axis=0)
            feature_id_train = np.concatenate(feature_id_train, axis=0)
            stacked_feature_id_train = np.hstack([feature_id_train, betas])

            logit_id_train = feature_id_train @ self.w.T + self.b

            self.u = -np.matmul(pinv(self.w), self.b)
            self.u = np.concatenate([self.u, np.zeros(betas.shape[1])])
    ...
    def postprocess(self, net: nn.Module, data: Any):
        ...
        betas = lime_explanation(net, data)

        stacked_feature_ood = np.hstack([feature_ood, betas])
