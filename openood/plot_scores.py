...
for name, function in utils.get_aggregate_functions():
    ...
    if near_auc > 0.5:
        print(
            f'{name}:{" "*spaces} Near-AUC {near_auc:.3f}   Far-AUC {far_auc:.3f} Corr: {correlations:.2f}'
        )
    else:
        print(
            f'{name}:{" "*spaces} Near-AUC {1 - near_auc:.3f}   Far-AUC {1 - far_auc:.3f} Corr: {correlations:.2f}    (if negated)'
        )
