...
if args.plot:
    mean_aurocs = [
        [dictionary[key]['near'] for key in dictionary]
    ...
    indices = [
        ...
        # for x in ['\\ac{mls}', '\\ac{msp}', 'CV', 'RMD', 'QCD']
    ...
    if args.pgf:
        pgf_filename = f'../master/figure/{args.dataset}_heatmap.pgf'
        plt.savefig(pgf_filename)
        plt.clf()
        print(f'scp uio:master/master/figure/{args.dataset}_heatmap.pgf figure/')
        print(f'scp uio:master/master/figure/{args.dataset}_heatmap-img0.png figure/')
        if not os.path.isfile(pgf_filename):
            ...
            new_contents = file_contents.replace(
                f'{args.dataset}_heatmap-img0.png',
                f'figure/{args.dataset}_heatmap-img0.png',
            )
...
mean_values_dict.pop('\\ac{mls}')
mean_values_dict.pop('\\ac{msp}')
...
if args.pgf:
    ...
    plt.clf()
    print(
        f'scp uio:master/master/figure/{args.dataset}_all_metrics_barplot.pgf figure/'
    )
...
if args.pgf:
    plt.savefig(f'../master/figure/{args.dataset}_all_generators_barplot.pgf')
    plt.clf()
    print(
        f'scp uio:master/master/figure/{args.dataset}_all_generators_barplot.pgf figure/'
    )
