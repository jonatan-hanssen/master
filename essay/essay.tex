...
\cite{vim} attempts to improve OOD detection by calculating a score based on the feature, the logit and the softmax probability, as opposed to just one of them. By looking at all three elements in conjunction they see an increase in performance over models which only rely on a single input source (such as the previously mentioned ODIN).

To do this, they propose the Virtual Logit
