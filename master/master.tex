...
% \begin{figure}[h]
% \centerline{\includegraphics[width=3.25in]{figure/ood_separation.png}}
...
% More generally, classification based methods attempt to find a metric which separates the distribution of ID samples from OOD samples. That is, we want a metric which outputs values in a certain range for ID samples and values in a different range for OOD samples, with as little overlap as possible. Figure \ref{ood_metric} shows the probability density for an unspecified metric for both OOD and ID samples. The goal of OOD detection is to find metrics or training methods which make these probability densities have as little overlap (the shaded region), such that they are easily separated by .
