...
With a thorough introduction to both XAI and OOD detection, I shall present the datasets that will be used to test the new method that I introduce in chapter \ref{chapter:newmethod}. The two datasets used are OpenOOD (\cite{openood}) and HyperKvasir (\cite{hyperkvasir}). OpenOOD is a comprehensive benchmark which will test the new method's performance in a wide range of OOD scenarios, while HyperKvasir is a medicinal dataset which will be used to test the method in the specific use case of medicinal OOD detection.
...
\subsubsection{HyperKvasir} % \label{chapter:openood}
