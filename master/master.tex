...
\begin{enumerate}
    \item Develop an \ac{ood} detection framework which relies solely on \ac{xai} explanations, and compare methods developed under this framework to traditional \ac{ood} detection methods.
    \item Develop one or more \ac{ood} detection frameworks that combine traditional and \ac{xai} based methods in different ways, and compare methods developed under this framework to traditional \ac{ood} detection methods.
\end{enumerate}
...
\begin{itemize}
    ...
    \item Saliency values can also be appended directly to the model logits in \ac{ood} detection methods such as \ac{vim}, leading to statistically significant improvements over using the method without \ac{xai} saliency values in some cases. However, the improvement in this case is not a strong as when using saliency aggregation.
\end{itemize}
...
Finally, let us consider both validation benchmarks together. Figure \ref{fig:both_heatmap} shows the average performance over both Near-\ac{ood} and Far-\ac{ood} for both ImageNet200 and CIFAR10. Using this heatmap, we can select the most promising combinations to use for the final \ac{ood} detectors on the test set.
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[ImageNet200 Saliency Aggregation Bootstrap]{Barplot of average \ac{auroc} scores for Saliency Aggregation on the bootstrapped ImageNet200 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[CIFAR10 Saliency Aggregation Bootstrap]{Barplot of average \ac{auroc} scores for Saliency Aggregation on the bootstrapped CIFAR10 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[CIFAR100 Saliency Aggregation Bootstrap]{Barplot of average \ac{auroc} scores for Saliency Aggregation on the bootstrapped CIFAR100 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[CIFAR10 Saliency Aggregation Plus Logits Bootstrap]{Barplot of average \ac{auroc} scores for Saliency Aggregation Plus Logits on the bootstrapped CIFAR10 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
Finally, we inspect the results for \ac{gbp}Norm+Logits, which are very impressive. Here, we see substantial improvements over both baselines of several percentage points on both Near- and Far-\ac{ood}. Furthermore, these results are pretty close to the \ac{sota} amongst methods which do not require retraining of the network, with \texttt{COMBOOD} being the leader at 91.13/94.65 Near-\ac{ood}/Far-\ac{ood} performance.

\subsubsection{CIFAR100}

Next, we turn to CIFAR100. Again we find that several of the \ac{xai} based methods outperform the baselines on Far-\ac{ood}, while the performance on Near-\ac{ood} is competitive with baseline methods.

\begin{figure}[H]
    \begin{center}
        \input{figure/cifar100_salpluslogit_bootstrap_barplot.pgf}
    \end{center}
    \caption[CIFAR100 Saliency Aggregation Plus Logits Bootstrap]{Barplot of average \ac{auroc} scores for Saliency Aggregation Plus Logits on the bootstrapped CIFAR100 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
\begin{tabular}{ |c|c!{\vrule width 1pt}c|c!{\vrule width 1pt}c|c| }
    ...
    \rowcolor{far!50}
    Far-OOD & 92.17 & +2.656 & 4.5e-19 *** & +7.160 & 7.7e-26 *** \\
    \hline
    \hline
    \multicolumn{6}{|c|}{GradCAMNorm} \\
    \hline
    \rowcolor{near!50}
    Near-OOD & 76.68 & +0.348 & 0.015 & +0.641 & 1.9e-04 *** \\
    \rowcolor{far!50}
    Far-OOD & 91.25 & +1.739 & 1.2e-15 *** & +6.243 & 1.3e-24 *** \\
    \hline
    \hline
    \multicolumn{6}{|c|}{IGMean} \\
    \hline
    ...
    \rowcolor{far!50}
    Far-OOD & 88.85 & -0.665 & 1.000 & +3.839 & 2.6e-20 *** \\
    \hline
    \hline
    \multicolumn{6}{|c|}{GBPNorm} \\
    \hline
    \rowcolor{near!50}
    Near-OOD & 78.25 & +1.920 & 1.6e-11 *** & +2.212 & 1.4e-12 *** \\
    \rowcolor{far!50}
    Far-OOD & 93.83 & +4.320 & 1.2e-23 *** & +8.824 & 4.3e-28 *** \\
    \hline
    \end{tabular}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[ImageNet200 SaliencyVIM Bootstrap]{Barplot of average \ac{auroc} scores for SaliencyVIM on the bootstrapped ImageNet200 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[CIFAR10 SaliencyVIM Bootstrap]{Barplot of average \ac{auroc} scores for SaliencyVIM on the bootstrapped CIFAR10 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[ImageNet1K SaliencyVIM Bootstrap]{Barplot of average \ac{auroc} scores for SaliencyVIM on the bootstrapped ImageNet1K test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[CIFAR100 SaliencyVIM Bootstrap]{Barplot of average \ac{auroc} scores for SaliencyVIM on the bootstrapped CIFAR100 test set. 95\% confidence intervals are plotted as whiskers. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
