...
--++ (0,#3,0) --++ (0,0,-#4) -- cycle;
\path[decorate,decoration={text effects along path,text={Conv}}] (#2/2,{2+(#3-2)/2},0) -- (#2/2,0,0);
}
}}
...
After several such convolutions, we can flatten the output, either by aggregating each channel using \ac{gap} or a similar method, or we can simply flatten all dimensions and consider the three dimensional feature map as a long vector of shape $C \times H \times W$. By doing this, we can pass the output.
...
{
    \draw pic (box1-\Y) at (\Y,-\X/2,0) {fake box=white!70!gray with dimensions {1/\X/\X} and {2*\X} and 1*\X};
}
...
\Col!50!white] (0,0.25) rectangle (1.5,-0.75);}
...
\end{tikzpicture}
    ...
    \end{center}
    \caption[Convolution example]{Figure showing a convolutional kernel applied to a simple 1-channel image.}
    \label{fig:conv}
\end{figure}
