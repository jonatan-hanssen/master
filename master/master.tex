...
\begin{abstract}
    ...
    \ac*{xai} is another field which is concerned with making \acs*{ai} models more trustworthy and robust. Like \acs*{ood} detection, this field has seen increased interest as \ac{ai} methods are used for a wider array of tasks than previously. \acs*{xai} methods attempt to understand how \acs*{ai} models come to a particular decision. To do this, they use gradient information, counterfactuals, and model internals to create explanations of predictions which can be inspected by humans. These methods, although they are intended primarily for human inspection, may also capture information from \acs*{ai} models which could be used to detect \acs*{ood} data points. To date, only one work has attempted to combine \acs*{xai} and \acs*{ood} detection in this way, with limited results. This thesis further explores the possibility of using \acs*{xai} explanations for \ac{ood} detection.
...
\subsection{XAI methods adapted to images: Saliency maps and segmentation} \label{section:saliencymapbackground}
...
Although \ac{cam} is an intuitive and effective method of visualizing the inner workings of a \ac{cnn}, it has some downsides. Firstly, it is highly model dependent, requiring that the model only have a single dense layer after the convolutions, and that \ac{gap} is used for flattening. Although this is a common architecture among \ac{sota} \ac{cnn} classifiers, this still places a limit on what models can be used, or requires the simplification of models that use more than a single dense layer. \cite[4]{cam} notes a 1-2\% drop in classification performance when performing this simplification. Secondly, the output of \ac{cam} is simply a weighted sum of all the feature maps after the final convolutional layer. As we move deeper in a \ac{cnn}, we reduce the spatial resolution by downsampling, while increasing the number of channels (increasing the depth of the output while reducing the height and width). Because of this, the \ac{cam} will have a drastically lower resolution than the original image, often less than $10 \times 10$, while the input image may be hundreds of pixels in both dimensions. Because of this, \ac{cam} can only show general areas, as opposed to pixel wise explanations.
\\
...
\begin{figure}[hbtp]
    ...
    \end{tikzpicture}
    \caption[Null-space noise visualization]{Figure showing the findings of \cite{nusa} visually. By sampling null space noise from the network, we can create an image (the image to the right) which is completely distorted, but which is indistinguishable for the network and given the exact same prediction as if the noise was not present. Original image taken from the ImageNet dataset.}
    \label{dog}
    \end{center}
\end{figure}
...
For integrated gradients, the only choice that must be made is the choice of baseline. An ideal baseline should convey no information, and should ideally lead to an output of zero from the network. As \cite{sturmfels2020visualizing} shows, there are no perfect choices for such a baseline, as all methods carry some assumption about our dataset and what it means for an input to "convey no information". However, the common choice for image classification tasks is the zero vector (a completely gray image, if we assume ResNet normalization), as recommended by \cite{integratedgradients}. As such, this is the baseline I have used.
...
\begin{tabular}{ |p{5.1em}|c c c c c c|c c c| }
    ...
    \hline
    \end{tabular}
    \caption[Average \ac{auroc} scores over all \ac{xai} saliency methods on ImageNet200]{Average \ac{auroc} scores for all \ac{xai} saliency methods on ImageNet200. The highest non-baseline value for Near- and Far-\ac{ood} is highlighted in bold.}
...
These results are very interesting, considering that \cite{martinez} found that \ac{xai} saliency maps had very little discriminatory power. The difference between magnitude aggregations and magnitude invariant dispersion aggregations give a convincing argument to explain these poor results: Normalizing saliency maps removes vital information required for \ac{ood} detection, and the magnitude invariant spread of saliency maps is not particularly different between \ac{id} and \ac{ood} data sets. Inspecting the code released as part of the work of \cite{martinez}, we find that they indeed normalize their saliency maps prior to performing clustering,\footnote{\url{https://github.com/aitor-martinez-seras/OOD-XAI/blob/main/utils/utils.py}, line 84} which we have seen has very detrimental effects on the separability of \ac{id} and \ac{ood} data points.
...
\begin{tabular}{ |c|c!{\vrule width 1pt}c|c!{\vrule width 1pt}c|c| }
    ...
    \hline
    \end{tabular}
    \caption[Wilcoxon signed-rank test for Saliency Aggregation on Imagenet]{Results of performing a Wilcoxon signed-rank test against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on Imagenet, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
...
\begin{tabular}{ |c|c!{\vrule width 1pt}c|c!{\vrule width 1pt}c|c| }
    ...
    \hline
    \end{tabular}
    \caption[Wilcoxon signed-rank test for SaliencyAggregation plus Logit on ImageNet200]{Results of performing a Wilcoxon signed-rank test against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
...
\begin{tabular}{ |c|c!{\vrule width 1pt}c|c!{\vrule width 1pt}c|c| }
    ...
    \hline
    \end{tabular}
    \caption[Wilcoxon signed-rank test for SaliencyAggregation plus Logit on Imagenet]{Results of performing a Wilcoxon signed-rank test against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on Imagenet, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
...
\begin{tabular}{ |c|c!{\vrule width 1pt}c|c!{\vrule width 1pt}c|c| }
    ...
    \hline
    \end{tabular}
    \caption[Wilcoxon signed-rank test for SaliencyAggregation plus Logit on CIFAR100]{Results of performing a Wilcoxon signed-rank test against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on CIFAR100, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
...
The performance of \ac{gbp}Norm+Logit is a particularly interesting example, beating both baselines by several percentage points on all four benchmarks on Far-\ac{ood}. In fact, the performance is actually quite close to \ac{sota} scores on all benchmarks: Table \ref{table:gbpnorm} shows the difference in \ac{auroc} scores between \ac{gbp}Norm+Logit and \ac{sota} methods as reported by \cite{openood, openood15}.\footnote{\url{https://zjysteven.github.io/OpenOOD/}}
...
\begin{tabular}{ |c|c|c|c| }
    ...
    \hline
    \end{tabular}
    \caption[Wilcoxon signed-rank test for SaliencyVIM on Imagenet]{Results of performing a Wilcoxon signed-rank test against \ac{vim}, showing the mean \ac{auroc} over 10 runs on Imagenet, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
