...
\textbf{In what ways can methods from the field of Explainable Artificial Intelligence be used to improve Out-of-Distribution Detection?}
\\
...
In this thesis, the goal has been to investigate whether \ac{xai} methods can be used for \ac{ood} detection, and to introduce proof-of-concept \ac{ood} detection models which use \ac{xai} explanations as part of their functioning. As part of this work, I have introduced three forms of \ac{xai} inspired \ac{ood} detection models, which use the saliency maps (heatmaps) generated by \ac{xai} methods applied to images: {\it Saliency Aggregation}, {\it Saliency Aggregation+Logits} and {\it Saliency\acs*{vim}}.

Through the testing of these three methods, I show that \ac{xai} methods can indeed be used for \ac{ood} detection, contrary to the results found by previous research \cite{martinez}. The key takeaways of the thesis are as follows:
...
\begin{itemize}
    ...
    \item Depending on the choice of \ac{xai} method, \ac{xai} saliency maps output values which are relatively uncorrelated with baseline \ac{ood} scores. This means that their outputs can be combined with other \ac{ood} detection methods to increase \ac{ood} detection performance. By combining \ac{xai} saliency aggregates with \ac{mls} by a simple addition of metrics, I find that the performance is increased by several percentage points over the baselines in several cases, and in one case performs quite close to the \ac{sota} amongst \ac{ood} detection models which do not retrain the underlying classifier.
    \item Saliency values can also be appended directly to the model logits in \ac{ood} detection methods such as \ac{vim}, leading to statistically significant improvements over using the method without \ac{xai} saliency values.
\end{itemize}

In addition, I have had a pull request merged in the codebase for OpenOOD, the definitive \ac{ood} detection framework and benchmarking tool. By improving the code used when benchmarking new \ac{ood} detection methods, I have contributed to the broader field of \ac{ood} detection.

\section{Thesis Outline}

Chapter \ref{chapter:background} gives a short introduction to machine learning, followed by a deeper look at the fields of \ac{xai} and \ac{ood} detection. Chapter \ref{chapter:methodology} introduces my methodology; the methods I will use to compare and contrast \ac{xai} explanations on \ac{id} and \ac{ood} data, as well as a handful of \ac{ood} detection methods which integrate explanations into their functioning. Furthermore, I introduce the different datasets which will be used in chapter \ref{chapter:experiments}, among them the HyperKvasir gastrointestinal dataset \cite{hyperkvasir}. \ref{chapter:experiments} will first go into the results of the comparison between \ac{id} and \ac{ood} explanations, detailing the differences between different methods. Then, I will report the performance of the different \ac{ood} detection algorithms I have developed, on a handful of datasets, including HyperKvasir, CIFAR10 and ImageNet. After the experiments follow a discussion (chapter \ref{chapter:discussion}), where I reflect on the benefits and drawbacks of using \ac{xai} methods for \ac{ood} detection, and summarize the results from \ref{chapter:experiments}. Finally, the conclusion (chapter \ref{chapter:conclusion}) concludes.
...
\begin{figure}[H]
    ...
    \end{center}
    \caption[Segmentation comparison]{Figure showing a comparison between generating saliency maps using the \ac{lime} algorithm on rectangular and superpixel regions}
...
In 2021, \cite{oodoverview} defined a generalized \ac{ood} detection framework, and in 2023 \cite{openood} introduced a comprehensive benchmark called OpenOOD, which evaluates all relevant \ac{ood} methods under this framework. Prior to this work, different methods were tested on different datasets, with different image preprocessing procedures, and with other externalities which inhibited effective comparison between methods \cite{openood}.

OpenOOD includes 11 different benchmarks across Anomaly Detection, Open Set Recognition and \ac{ood} detection, three fields which are very closely related. Of these, 6 benchmarks are used to test methods for \ac{ood} detection, which are the ones I will be concerned with. Each benchmark is defined by an \ac{id} dataset, with 6 or more corresponding \ac{ood} datasets, separated into Near-\ac{ood} and Far-\ac{ood}. For each benchmark, \ac{auroc}, \ac{aupr} and \ac{fpr95} is reported over all \ac{ood} datasets, and Near-\ac{ood} \ac{auroc} is used to rank methods against each other.
...
\subsubsection{Baseline models} \label{ood_baseline}
...
\section{Proposed \ac{xai} methods for \ac{ood} detection} \label{section:methods}

With the preceding intuitions in mind, I present three proposed methods for \ac{ood} detection which utilize \ac{xai} methods as part of their detection pipeline.

\subsection{Stand-alone saliency method: Aggregate of Saliency} \label{section:saliencyagg_method}
...
% As in the previous section, we assume we have a model $f: \bm{x} \to \R^C$, an \ac{xai} saliency mapping method $s: (f, \bm{x}) \to \R^{H_s \times W_s}$, and an aggregation function $A: \bm{x} \rightarrow \R$. Given that the logit and the aggregate of saliencies can have wildly different magnitudes, we should also normalize them so that they have about the same magnitude and spread. To do this, we can simply subtract the mean and divide by the standard deviation over an entire validation dataset, for each metric. Thus, we further assume we have mean and standard deviations of the max logit over an \ac{id} validation set $\mu_{\text{MLS}}$ and $\sigma_{\text{MLS}}$, as well as the mean and standard deviation of the aggregate of saliencies $\mu_{\text{Agg}}$ and $\sigma_{\text{Agg}}$. The \ac{ood} score of a given sample is then given by the following function:
%
% \begin{equation}
% \text{LogitSaliency}(\bm{x}; s, A)= \frac{A(s(\bm{x}, f)) - \mu_{\text{Agg}}}{\sigma_{\text{Agg}}} + \frac{\max_i S(\bm{x}) - \mu_{\text{MLS}}}{\sigma_{\text{MLS}}}
% \label{eq:salmax}
% \end{equation}
%
% As we can see, this function will increase as either the logit or saliency The \ac{ood} detector then has the following form, given a threshold $\delta$:
%
%
% \begin{align}
%     g(\bm{x}; s, A, \delta)=\begin{cases} 
%         \text{in } & \text{LogitSaliency}(\bm{x}; s, A) \ge \delta \\
%         \text{out} & \text{LogitSaliency}(\bm{x}; s, A) < \delta
%    \end{cases}
% \label{eq:aggregate}
% \end{align}
...
\subsection{Overview of testing environment}

Before introducing the specific datasets, I will describe the testing environment that has been used, so that it is clear for the reader how the \ac{ood} detection metrics are calculated.

For a given \ac{id} dataset, we have a series of \ac{ood} datasets, which are all semantically shifted in relation to the \ac{id} dataset. These \ac{ood} datasets are categorized into Near- and Far-\ac{ood}, depending on the degree of their semantic shift. The \ac{id} dataset is split into a train, validation and test set, while the \ac{ood} datasets are split into validation and test sets. A network is trained on the train \ac{id} set.
...
The theory of each \ac{xai} saliency method used in this thesis has been described in section \ref{section:xaimethodsbackground} in chapter \ref{chapter:background}. In the following section, I will describe the specifics of how I have applied each method in my efforts to better align them with the goal of separating \ac{id} and \ac{ood} data points.
...
This chapter contains my findings. In section \ref{section:saliencyagg}, I show how different forms of aggregation over saliency maps separate \ac{id} and \ac{ood} data points. This step is important, because it informs the choice of aggregation used in the Aggregation-of-Saliency \ac{ood} detection method. I report \ac{auroc} for different forms of aggregation over the three datasets. This is done on the validation sets, to avoid biasing the final results.

In section \ref{section:results}, \ac{ood} detection on the test sets is performed using the three methods I have introduced in section \ref{section:methods}. The \ac{auroc} and \ac{fpr95} for each method on each dataset is reported. The choice of aggregation and \ac{xai} method is informed by the findings from the preceding section. In addition, I perform statistical analyses to investigate whether my \ac{xai} \ac{ood} detection methods outperform baseline methods.
...
Figure \ref{fig:imagenet200_all_metrics_barplot} shows the average Near- and Far-\ac{auroc} for magnitude aggregations, as well as the baselines. From this, we see that while the average scores are below the baselines, they are far better than what was achieved by \cite{martinez}, and in general show that there is a clear distinction between the magnitudes of \ac{id} and \ac{ood} saliency maps. Given the poor scores of the statistical dispersion methods, I do not plot them.
...
Finally, figure \ref{fig:imagenet200_heatmap} shows a heatmap of the average performance over both Near-\ac{ood} and Far-\ac{ood} for all combinations of aggregation and \ac{xai} saliency method. In addition, the last row and column shows the average performance over aggregation and \ac{xai} method, respectively. From this we can get a visual overview over what combinations perform well.
...
Saliency maps made from \ac{gradcam}, with their mathematical equivalence to the maximum logit when applying the mean, unsurprisingly do not suffer from the same problems as occlusion. As we can see from table \ref{table:cifar10_gradcam_metrics}, the results for mean are equivalent to the maximum logit.
...
% With the results from all \ac{xai} saliency methods reported, we may now look at the 
Figure \ref{fig:cifar10_all_metrics_barplot} shows the average scores for each aggregation over the 5 differ \ac{xai} saliency methods. From this we see that again, vector norm performs the best overall, and occlusion the worst. We can also see that the drop in comparison to the baselines is higher on CIFAR10 than on ImageNet200, possibly due to the reduction in resolution. Another possible reason could be that the CIFAR benchmark is more difficult than ImageNet for \ac{ood} detection algorithms in general, as reported by \ac{openood}.
...
\begin{tabular}{ |p{5.1em}|c c|c c c c c c|c c c| }
    ...
    \hline
    \rowcolor{near!50}
    Near-\ac{ood} \ac{auroc} & 86.7 & 87.7 & 74.1 & 68.0 &\textbf{ 80.6 }& 74.1 & 77.1 & 76.2 & 65.8 & 61.6 & 57.6  \\
    \hline
    \rowcolor{far!50}
    Far-\ac{ood} \ac{auroc} & 91.4 & 90.8 & 75.4 & 72.1 &\textbf{ 81.6 }& 76.1 & 77.7 & 79.8 & 66.2 & 65.0 & 58.2  \\
    \hline
    Mean \ac{auroc} & 89.1 & 89.2 & 74.8 & 70.1 &\textbf{ 81.1 }& 75.1 & 77.4 & 78.0 & 66.0 & 63.3 & 57.9  \\
    \hline
    \end{tabular}
    \caption[Average \ac{auroc} scores over all \ac{xai} saliency methods on CIFAR10]{Average \ac{auroc} scores for all \ac{xai} saliency methods on CIFAR10. The highest non-baseline value for Near- and Far-\ac{ood} is highlighted in bold. $\downarrow$ denotes that \ac{id} data points more often have a lower score with this aggregation, and thus the output values have been negated (as described in section \ref{section:aurocfpr95})}
...
\subsection{Overall results on both datasets}

Finally, let us consider both datasets together. Figure \ref{fig:both_heatmap} shows the average performance over both Near-\ac{ood} and Far-\ac{ood} for both ImageNet200 and CIFAR10. Using this heatmap, we can select the most promising combinations to use for the final \ac{ood} detectors on the test set.
...
%     \hline
%     \rowcolor{near!50}
%     Near-\ac{ood} \ac{auroc} & 85.0 & 85.6 & 74.7 & 65.5 &\textbf{ 77.9 }& 73.2 & 74.8 & 73.8 & 60.5 & 58.1 & 55.0  \\
%     \hline
%     \rowcolor{far!50}
%     Far-\ac{ood} \ac{auroc} & 91.4 & 90.5 & 79.9 & 69.3 &\textbf{ 85.1 }& 82.5 & 83.3 & 79.9 & 60.1 & 63.6 & 57.2  \\
%     \hline
%     Mean \ac{auroc} & 88.2 & 88.1 & 77.3 & 67.4 &\textbf{ 81.5 }& 77.9 & 79.0 & 76.9 & 60.3 & 60.8 & 56.1  \\
%     \hline
%     Correlation with \ac{mls}& - & - & 0.49 & 0.34 & 0.46 & 0.32 & 0.41 & 0.44 & -0.09 & 0.00 & -0.06  \\
%     \hline
%     Correlation with \ac{msp}& - & - & 0.40 & 0.27 & 0.39 & 0.29 & 0.35 & 0.37 & -0.07 & 0.01 & -0.05  \\
%     \hline
%     \end{tabular}
...
\end{figure}
 ...
 As we can see, occlusion has consistently underperformed on both datasets, and as such I will not use this \ac{xai} method in the following sections. For the remaining four \ac{xai} methods, I select the highest performing aggregation on both of the validation sets for use on the test set. From the heatmap, we can see that this results in using vector norm for \ac{lime}, \ac{gradcam} and \ac{gbp}, and mean for integrated gradients. In the next section, I will denote these combinations as \ac{lime}Norm, \ac{gradcam}Norm, IGMean and \ac{gbp}Norm.
...
%     \hline
%     \rowcolor{near!50}
%     Near-\ac{ood} \ac{auroc} & 80.9 & 80.2 &\textbf{ 69.3 }& 64.8 & 67.0 & 63.2 & 64.4 & 67.9 & 60.1 & 59.2 & 55.2  \\
%     \hline
%     \rowcolor{far!50}
%     Far-\ac{ood} \ac{auroc} & 79.3 & 77.2 &\textbf{ 72.0 }& 68.0 & 69.1 & 66.2 & 67.0 & 70.7 & 60.5 & 55.3 & 57.4  \\
%     \hline
%     Mean \ac{auroc} & 80.1 & 78.7 &\textbf{ 70.6 }& 66.4 & 68.1 & 64.7 & 65.7 & 69.3 & 60.3 & 57.3 & 56.3  \\
%     \hline
%     \end{tabular}
%     \caption[Average \ac{auroc} scores over all \ac{xai} saliency methods on CIFAR100]{Average \ac{auroc} scores for all \ac{xai} saliency methods on CIFAR100. The highest non-baseline value for Near- and Far-\ac{ood} is highlighted in bold. $\downarrow$ denotes that \ac{id} data points more often have a lower score with this aggregation, and thus the output values have been negated (as described in section \ref{section:aurocfpr95})}
%     \label{table:cifar100_all_metrics}
% \end{center}
% \setlength\tabcolsep{6pt}
% \end{table}
%
...
%     \hline
%     \rowcolor{near!50}
%     Near-\ac{ood} \ac{auroc} & 76.4 & 75.8 &\textbf{ 66.4 }& 57.9 & 66.3 & 63.7 & 62.7 & 64.3 & 53.3 & 52.3 & 51.1  \\
%     \hline
%     \rowcolor{far!50}
%     Far-\ac{ood} \ac{auroc} & 89.6 & 85.3 & 76.2 & 61.6 & 84.8 &\textbf{ 85.2 }& 83.3 & 74.4 & 57.6 & 59.6 & 58.2  \\
%     \hline
%     Mean \ac{auroc} & 83.0 & 80.6 & 71.3 & 59.8 &\textbf{ 75.6 }& 74.5 & 73.0 & 69.3 & 55.5 & 56.0 & 54.6  \\
%     \hline
%     \end{tabular}
%     \caption[Average \ac{auroc} scores over all \ac{xai} saliency methods on Imagenet]{Average \ac{auroc} scores for all \ac{xai} saliency methods on Imagenet. The highest non-baseline value for Near- and Far-\ac{ood} is highlighted in bold. $\downarrow$ denotes that \ac{id} data points more often have a lower score with this aggregation, and thus the output values have been negated (as described in section \ref{section:aurocfpr95})}
%     \label{table:imagenet_all_metrics}
% \end{center}
% \setlength\tabcolsep{6pt}
% \end{table}
%
% \begin{figure}[H]
%     \begin{center}
%         \input{figure/imagenet_all_generators_barplot.pgf}
%     \end{center}
%     \caption[Average scores]{Barplot of average \ac{auroc} scores on CIFAR10. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
%     \label{fig:imagenet_all_generators_barplot}
% \end{figure}
%
% \begin{figure}[H]
%     \begin{center}
%         \input{figure/imagenet_all_metrics_barplot.pgf}
%     \end{center}
%     \caption[Average scores]{Barplot of average \ac{auroc} scores on CIFAR10. Note that the y-axis starts at 0.50, the practical floor for an \ac{auroc} score.}
%     \label{fig:imagenet_all_metrics_barplot}
% \end{figure}


% \subsection{Overall conclusion from Data Analysis}
%
% From the results above, we have found that saliency aggregations have definite potential to differentiate \ac{id} and \ac{ood} data. Based on the overall results, it seems that the vector norm performs the best on both datasets, while the maximum and range also perform reasonably well, although their performance on CIFAR10 was below that of the vector norm. In terms of \ac{xai} saliency methods, it seems that all gradient based methods perform well on both datasets.

% TODO: put in all methods averaged over both datasets

\section{Evaluation of XAI OOD detectors} \label{section:results}

This section contains the final tests conducted on the test set, using the \ac{xai} methods that performed the best on the validation set, as described in the preceding section. This section will be divided into three parts, corresponding to the three proposed \ac{ood} detection methods introduced in the methodology: Saliency Aggregation, Saliency Aggregation plus Logits and SaliencyVIM. Each of these sections will detail the performance of the corresponding method on the test set, and statistically compare the results to the baseline methods. The statistical analysis is based on ten bootstraps of the test set, which has been performed on each method and the baselines, as mentioned in chapter \ref{chapter:methodology}, the threshold for statistical significance is set at 0.05.

\subsection{Results for Saliency Aggregation} \label{section:salagg_results}

Saliency Aggregation, as described in \ref{section:saliencyagg_method}, is the first \ac{ood} detection method that will be tested. As we have seen from the validation set, saliency aggregation on its own can sometimes perform on par with the baselines on Near-\ac{ood} and can sometimes surpass the baselines on Far-\ac{ood}. In this section, the generation of saliency maps and aggregation will be done again on ten bootstrapped test sets, and \ac{auroc} scores will be calculated.

\subsubsection{ImageNet200}

As in the preceding section, we first investigate the results on ImageNet200. Figure \ref{fig:imagenet200_salagg_bootstrap_barplot} shows the mean \ac{auroc} for the baselines and the three combinations of \ac{xai} methods and aggregation functions mentioned above. In addition, the confidence intervals for each mean value is plotted as whiskers.
...
As we can see, the vector norm of \ac{lime}, \ac{gradcam} and \ac{gbp} actually outperforms the baselines on Far-\ac{ood}, while the mean of the integrated gradients is about on par with the Far-\ac{ood} performance of \ac{msp}. When it comes to Near-\ac{ood}, only the norm of \ac{gradcam} can compete with the baselines.

Looking at each combination individually and starting with \ac{lime}Norm, we see from table \ref{table:imagenet200_lime_norm_salagg_ttest} that \ac{lime}Norm sees highly statistically significant improvements on a number of Far-\ac{ood} datasets, leading to a statistically significant improvement on Far-\ac{ood} overall. These results show that \ac{xai} explanations can indeed be used to perform \ac{ood} detection.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 76.60 & -3.504 & 1.000 & -3.826 & 1.000 \\
    \hline
    \rowcolor{near!50}
    NINCO & 85.31 & -0.080 & 0.667 & -0.833 & 1.000 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 80.95 & -1.792 & 1.000 & -2.329 & 1.000 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 93.16 & -0.504 & 0.999 & +0.168 & 0.115 \\
    \hline
    \rowcolor{far!50}
    Textures & 92.91 & +2.489 & <0.001 *** & +4.476 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 91.11 & +1.354 & <0.001 *** & +1.941 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 92.39 & +1.113 & <0.001 *** & +2.195 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for LIMENorm saliency aggregation on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of LIMENorm saliency aggregation against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_lime_norm_salagg_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

Following \ac{lime}, we look at \ac{gradcam}Norm. From table \ref{table:imagenet200_gradcam_norm_salagg_ttest} that \ac{gradcam}Norm sees statistically significant improvements over the baselines on all datasets except for SSB\_HARD and NINCO (the Near-\ac{ood} datasets) against \ac{msp}. These results, along with the results above, seem to indicate that \ac{xai} saliency maps have some advantage when it comes to detecting Far-\ac{ood} data samples.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 80.51 & +0.405 & 0.014 * & +0.084 & 0.308 \\
    \hline
    \rowcolor{near!50}
    NINCO & 86.01 & +0.618 & <0.001 *** & -0.135 & 0.801 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 83.26 & +0.511 & 0.002 ** & -0.026 & 0.567 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 94.36 & +0.703 & <0.001 *** & +1.375 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Textures & 92.38 & +1.965 & <0.001 *** & +3.951 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 91.10 & +1.349 & <0.001 *** & +1.936 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 92.62 & +1.339 & <0.001 *** & +2.421 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for GradCAMNorm saliency aggregation on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of GradCAMNorm saliency aggregation against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_gradcam_norm_salagg_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

Next, table \ref{table:imagenet200_integratedgradients_mean_salagg_ttest} shows the results for IGMean. These results are quite poor we no significant results except for a statistically significant increase in \ac{auroc} over \ac{msp} on Textures.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 79.17 & -0.928 & 1.000 & -1.249 & 1.000 \\
    \hline
    \rowcolor{near!50}
    NINCO & 84.00 & -1.385 & 1.000 & -2.138 & 1.000 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 81.59 & -1.156 & 1.000 & -1.694 & 1.000 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 92.05 & -1.608 & 1.000 & -0.936 & 1.000 \\
    \hline
    \rowcolor{far!50}
    Textures & 89.87 & -0.550 & 0.973 & +1.436 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 89.23 & -0.522 & 0.998 & +0.065 & 0.343 \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 90.38 & -0.894 & 1.000 & +0.188 & 0.142 \\
    \hline
    \end{tabular}
    \caption[T-test for IGMean saliency aggregation on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of IGMean saliency aggregation against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_integratedgradients_mean_salagg_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

For \ac{gbp}Norm, the Near-\ac{ood} performance is significantly worse than the baselines, as was shown in figure \ref{fig:imagenet200_salagg_bootstrap_barplot}. However, the Far-\ac{ood} performance is statistically significantly better on all datasets except for iNaturalist against \ac{mls}.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 73.15 & -6.955 & 1.000 & -7.277 & 1.000 \\
    \hline
    \rowcolor{near!50}
    NINCO & 80.65 & -4.735 & 1.000 & -5.488 & 1.000 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 76.90 & -5.845 & 1.000 & -6.382 & 1.000 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 93.56 & -0.102 & 0.796 & +0.571 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Textures & 92.33 & +1.915 & <0.001 *** & +3.901 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 90.06 & +0.304 & 0.014 * & +0.891 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 91.98 & +0.706 & <0.001 *** & +1.788 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for GBPNorm saliency aggregation on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of GBPNorm saliency aggregation against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_gbp_norm_salagg_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}
...
Next, we turn our attention to CIFAR10. Figure \ref{fig:cifar10_salagg_bootstrap_barplot} shows the bootstrapped means and the confidence intervals. Here, we can see that the results are in general worse than the baselines, mirroring the initial findings from the validation dataset (section \ref{section:salagg_val_cifar10}). In this case, only \ac{gradcam}Norm can compete with the baseline methods.
...
Given the obviously poor results of the other methods, there is no need to report the results from the t-tests for \ac{lime}Norm, IGMean and \ac{gbp}Norm. However, \ac{gradcam}Norm may warrant some inspection. From table \ref{table:cifar10_gradcam_norm_salagg_ttest}, we see that even here, both the Near- and Far-\ac{ood} performance is relatively unimpressive.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    CIFAR100 & 85.78 & -0.075 & 0.663 & -1.193 & 1.000 \\
    \hline
    \rowcolor{near!50}
    TIN & 87.94 & -0.140 & 0.840 & -0.451 & 0.996 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 86.86 & -0.108 & 0.803 & -0.822 & 1.000 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    MNIST & 96.03 & -0.142 & 0.965 & +2.022 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    SVHN & 92.49 & +0.038 & 0.338 & +0.739 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Texture & 89.12 & +0.192 & 0.177 & -0.506 & 0.993 \\
    \hline
    \rowcolor{far!50}
    Places365 & 89.48 & -0.245 & 0.990 & -0.059 & 0.709 \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 91.78 & -0.039 & 0.644 & +0.549 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for GradCAMNorm saliency aggregation on CIFAR10]{Results of performing a t-test on the \ac{auroc} means of GradCAMNorm saliency aggregation against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on CIFAR10, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:cifar10_gradcam_norm_salagg_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}
...
Overall, the results from simply aggregating the saliency maps of different \ac{xai} methods are very promising, especially considering that the only other work which has attempted to use \ac{xai} saliency maps for \ac{ood} detection barely achieved \ac{auroc} scores above 0.50. These results show that \ac{xai} methods extract valuable information from the network, which can be used to effectively discriminate between \ac{id} and \ac{ood} data samples. The results on ImageNet200 are particularly interesting, as \ac{lime}Norm, \ac{gradcam}Norm and \ac{gbp}Norm all outperform the baselines on Far-\ac{ood}. The results on CIFAR10 are less impressive, and show that these methods may not achieve consistently good results on all datasets. However, it should be noted that this inconsistency is not unusual amongst \ac{ood} detectors, as one of the takeaways from from \cite{openood15} was there is "no single winner that always outperforms others across multiple data sets".

\subsection{Results for Saliency Aggregation Plus Logits} \label{section:salagglogits_results}

Given the fact that Saliency Aggregation has shown itself to be capable of differentiating \ac{id} and \ac{ood} samples in many cases, especially for Far-\ac{ood}, we might expect good results from Saliency Aggregate Plus Logits. As mentioned in section \ref{section:salpluslogitmethod}, \cite{combood} achieved \ac{sota} results on ImageNet200 when combining two complementary distance metrics. As we have seen from section \ref{section:salagg_results}, most \ac{xai} methods (except for \ac{gradcam}) have relatively low correlation with either \ac{msp} or \ac{mls}, which could give similar benefits as those found by \texttt{COMBOOD}.

\subsubsection{ImageNet200}

Figure \ref{fig:imagenet200_salpluslogit_bootstrap_barplot} shows the results of the 10 bootstraps. Right away we can see that the poor performances on Near-\ac{ood} have mostly disappeared when combining saliency aggregation with \ac{mls}. Furthermore \ac{gbp}Norm+Logits achieves a very impressive Far-\ac{ood} performance on ImageNet200. In addition, \ac{lime}Norm+Logits substantially improves on \ac{lime}Norm, and also beats the baselines on Far-\ac{ood}. \ac{gradcam}Norm+Logits also outperforms the baselines, but we should remember that \ac{gradcam}Norm also performed well, so these results may not be better than simply using \ac{gradcam}Norm alone. Let us look closer at the results, considering each method individually.
...
As we see from table \ref{table:imagenet200_lime_norm_salpluslogit_ttest}, \ac{lime}Norm is substantially improved by the addition of the \ac{mls}, and the combination of the two is statistically significantly better than \ac{mls} alone on both Near- and Far-\ac{ood}.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 79.96 & -0.139 & 0.780 & -0.460 & 0.993 \\
    \hline
    \rowcolor{near!50}
    NINCO & 86.96 & +1.569 & <0.001 *** & +0.816 & <0.001 *** \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 83.46 & +0.715 & <0.001 *** & +0.178 & 0.136 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 94.61 & +0.955 & <0.001 *** & +1.628 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Textures & 92.89 & +2.474 & <0.001 *** & +4.460 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 91.86 & +2.106 & <0.001 *** & +2.693 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 93.12 & +1.845 & <0.001 *** & +2.927 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for LIMENorm salpluslogit on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of LIMENorm salpluslogit against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_lime_norm_salpluslogit_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 80.45 & +0.344 & 0.033 * & +0.023 & 0.446 \\
    \hline
    \rowcolor{near!50}
    NINCO & 85.86 & +0.470 & 0.005 ** & -0.283 & 0.951 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 83.15 & +0.407 & 0.010 * & -0.130 & 0.791 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 94.15 & +0.490 & 0.001 ** & +1.162 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Textures & 91.52 & +1.105 & <0.001 *** & +3.091 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 90.58 & +0.829 & <0.001 *** & +1.416 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 92.09 & +0.808 & <0.001 *** & +1.890 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for GradCAMNorm salpluslogit on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of GradCAMNorm salpluslogit against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_gradcam_norm_salpluslogit_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

For IGMean+Logits, the performance is increased over IGMean on every single \ac{ood} dataset. This is most likely due to the extra information which is being added by including the logit. However, the performance is not better than simply using \ac{mls}. This is surprising, given that the performance of \ac{lime}Norm was far worse than IGMean, yet its performance when combined with \ac{mls} was far higher.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 79.80 & -0.298 & 0.943 & -0.619 & 0.999 \\
    \hline
    \rowcolor{near!50}
    NINCO & 84.92 & -0.471 & 0.992 & -1.224 & 1.000 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 82.36 & -0.384 & 0.982 & -0.922 & 1.000 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 93.06 & -0.595 & 0.999 & +0.077 & 0.312 \\
    \hline
    \rowcolor{far!50}
    Textures & 90.34 & -0.074 & 0.605 & +1.912 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 89.72 & -0.032 & 0.578 & +0.555 & 0.001 ** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 91.04 & -0.233 & 0.899 & +0.848 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for IGMean salpluslogit on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of IGMean salpluslogit against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_integratedgradients_mean_salpluslogit_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

Finally, we inspect \ac{gbp}Norm+Logits. As with IGMean+Logits and \ac{lime}Norm+Logits, the improvement over saliency aggregation is consistent over all \ac{ood} datasets. Here, the improvement is considerable, especially on Far-\ac{ood}, where the performance is actually quite close to the \ac{sota} models, which is around 0.95. However, the Near-\ac{ood} performance is not statistically significantly better than either \ac{mls} or \ac{msp}.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 79.74 & -0.359 & 0.973 & -0.681 & 1.000 \\
    \hline
    \rowcolor{near!50}
    NINCO & 86.18 & +0.794 & <0.001 *** & +0.041 & 0.402 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 82.96 & +0.217 & 0.099 & -0.320 & 0.971 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 95.75 & +2.089 & <0.001 *** & +2.762 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Textures & 93.48 & +3.060 & <0.001 *** & +5.046 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 92.64 & +2.882 & <0.001 *** & +3.469 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 93.95 & +2.677 & <0.001 *** & +3.759 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for GBPNorm salpluslogit on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of GBPNorm salpluslogit against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_gbp_norm_salpluslogit_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}
...
Next, we inspect the performance of \ac{gradcam}Norm+Logit. As in the previous section, the high correlation between the \ac{mls} and \ac{gradcam} saliencies means that this method does not perform any better than either \ac{mls} or \ac{gradcam}Norm.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    CIFAR100 & 85.82 & -0.030 & 0.567 & -1.148 & 1.000 \\
    \hline
    \rowcolor{near!50}
    TIN & 88.02 & -0.062 & 0.671 & -0.373 & 0.989 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 86.92 & -0.046 & 0.643 & -0.760 & 1.000 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    MNIST & 96.10 & -0.066 & 0.806 & +2.098 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    SVHN & 92.47 & +0.023 & 0.401 & +0.724 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Texture & 89.03 & +0.102 & 0.311 & -0.597 & 0.998 \\
    \hline
    \rowcolor{far!50}
    Places365 & 89.62 & -0.112 & 0.870 & +0.075 & 0.241 \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 91.81 & -0.013 & 0.550 & +0.575 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for GradCAMNorm salpluslogit on CIFAR10]{Results of performing a t-test on the \ac{auroc} means of GradCAMNorm salpluslogit against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on CIFAR10, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:cifar10_gradcam_norm_salpluslogit_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

Next; IGMean+Logit. Here, we see significant improvements over IGMean, as with ImageNet200. In addition, the method is statistically significantly better than both baselines on Far-\ac{ood}, and better than \ac{mls} on Near-\ac{ood}.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    CIFAR100 & 85.83 & -0.021 & 0.550 & -1.139 & 1.000 \\
    \hline
    \rowcolor{near!50}
    TIN & 88.58 & +0.501 & <0.001 *** & +0.190 & 0.106 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 87.21 & +0.240 & 0.028 * & -0.475 & 0.999 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    MNIST & 95.50 & -0.671 & 1.000 & +1.493 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    SVHN & 93.15 & +0.704 & <0.001 *** & +1.406 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Texture & 91.82 & +2.894 & <0.001 *** & +2.196 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Places365 & 89.39 & -0.339 & 0.999 & -0.152 & 0.927 \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 92.47 & +0.647 & <0.001 *** & +1.236 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for IGMean salpluslogit on CIFAR10]{Results of performing a t-test on the \ac{auroc} means of IGMean salpluslogit against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on CIFAR10, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:cifar10_integratedgradients_mean_salpluslogit_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

Finally, we inspect the results for \ac{gbp}Norm+Logits, which are very impressive. Here, we see substantial improvements over both baselines of several percentage points on all datasets except for MNIST. Furthermore, these results are pretty close to the \ac{sota} amongst methods which do not require retraining of the network, with \texttt{COMBOOD} being the leader at 91.13/94.65 Near-\ac{ood}/Far-\ac{ood} performance.
...
\begin{tabular}{ |c|c|c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    CIFAR100 & 88.78 & +2.923 & <0.001 *** & +1.805 & <0.001 *** \\
    \hline
    \rowcolor{near!50}
    TIN & 91.04 & +2.957 & <0.001 *** & +2.646 & <0.001 *** \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 89.91 & +2.940 & <0.001 *** & +2.225 & <0.001 *** \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{6}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    MNIST & 96.29 & +0.122 & 0.058 & +2.286 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    SVHN & 96.66 & +4.214 & <0.001 *** & +4.916 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Texture & 92.30 & +3.374 & <0.001 *** & +2.675 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Places365 & 93.45 & +3.720 & <0.001 *** & +3.907 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 94.68 & +2.858 & <0.001 *** & +3.446 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for GBPNorm salpluslogit on CIFAR10]{Results of performing a t-test on the \ac{auroc} means of GBPNorm salpluslogit against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on CIFAR10, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:cifar10_gbp_norm_salpluslogit_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}
...
For the final tests, we consider SaliencyVIM, the method described in section \ref{section:saliencyvim_method}. In this case, it does not make sense to use \ac{mls} or \ac{msp} as the baselines to compare against, since the method is based on \ac{vim}. Instead, we compare our results \ac{vim}, to see whether the addition of saliencies can improve this method.

\subsubsection{ImageNet200}

We start with ImageNet200. From figure \ref{fig:imagenet200_salvim_bootstrap_barplot}, we see that the differences are extremely small between the baseline and the \ac{xai} methods. However, the confidence intervals are also very small, which might mean that there are statistically significant performance increases. Thus, we look to the results of the t-tests.
...
As we see from table \ref{table:imagenet200_lime_Norm_salvim_ttest}, SaliencyVIM using \ac{lime} to generate saliencies leads to a statistically significant increase in performance on Far-\ac{ood}, but not on Near-\ac{ood}. These results mirror those of the previous sections, which in general have found that \ac{xai} saliency maps more often lead to improvements on Far-\ac{ood} than Near-\ac{ood}.
...
\begin{tabular}{ |c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 69.72 & +0.131 & 0.136 \\
    \hline
    \rowcolor{near!50}
    NINCO & 79.71 & +0.225 & 0.166 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 74.72 & +0.178 & 0.126 \\
    \hline
    \hline
    \rowcolor{far!50}
    ...
    \hline
    \rowcolor{far!50}
    iNaturalist & 88.27 & +0.428 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Textures & 92.70 & +0.186 & 0.035 * \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 85.53 & +0.280 & 0.004 ** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 88.83 & +0.298 & <0.001 *** \\
    \hline
    \end{tabular}
    \caption[T-test for LIMENorm salvim on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of LIMENorm salvim against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baseline method \ac{vim}, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_lime_Norm_salvim_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

Next, we inspect the results of the t-test when using \ac{gradcam} with Saliency\ac{vim}. Here, we see no improvements on either Near- or Far-\ac{ood}, showing that the choice of saliency generator is important when integrating \ac{xai} methods with traditional \ac{ood} detectors.
...
\begin{tabular}{ |c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    SSB\_HARD & 69.41 & -0.183 & 0.937 \\
    \hline
    \rowcolor{near!50}
    NINCO & 79.41 & -0.077 & 0.631 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 74.41 & -0.130 & 0.803 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{4}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    iNaturalist & 87.84 & -0.004 & 0.516 \\
    \hline
    \rowcolor{far!50}
    Textures & 92.51 & -0.003 & 0.511 \\
    \hline
    \rowcolor{far!50}
    OpenImage\_O & 85.19 & -0.059 & 0.743 \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 88.51 & -0.022 & 0.634 \\
    \hline
    \end{tabular}
    \caption[T-test for GradCAMNorm salvim on ImageNet200]{Results of performing a t-test on the \ac{auroc} means of GradCAMNorm salvim against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on ImageNet200, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:imagenet200_gradcam_Norm_salvim_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}
...
As we see from table \ref{table:cifar10_lime_Norm_salvim_ttest}, \ac{vim} integrated with \ac{lime} saliencies is indeed worse than the original \ac{vim}, on all \ac{ood} datasets. This is interesting, given the superior performance of Saliency\ac{vim} with \ac{lime} on Far-\ac{ood} datasets in ImageNet200. However, as mentioned previously, \cite{openood15} has found that this is quite common for \ac{ood} detectors, with no method outperforming all others on all benchmarks.
...
\begin{tabular}{ |c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    CIFAR100 & 85.90 & -1.252 & 1.000 \\
    \hline
    \rowcolor{near!50}
    TIN & 88.17 & -1.451 & 1.000 \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 87.03 & -1.352 & 1.000 \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{4}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    MNIST & 91.82 & -2.441 & 1.000 \\
    \hline
    \rowcolor{far!50}
    SVHN & 94.22 & -0.073 & 0.824 \\
    \hline
    \rowcolor{far!50}
    Texture & 94.07 & -0.669 & 1.000 \\
    \hline
    \rowcolor{far!50}
    Places365 & 87.21 & -1.857 & 1.000 \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 91.83 & -1.260 & 1.000 \\
    \hline
    \end{tabular}
    \caption[T-test for LIMENorm salvim on CIFAR10]{Results of performing a t-test on the \ac{auroc} means of LIMENorm salvim against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on CIFAR10, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:cifar10_lime_Norm_salvim_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}

Finally, Saliency\ac{vim} with \ac{gradcam} outperforms \ac{vim} on both Near- and Far-\ac{ood}, with statistically significant improvements on all \ac{ood} datasets except for SVHN and Texture.
...
\begin{tabular}{ |c|c|c|c| }
    ...
    \hline
    \rowcolor{near!50}
    CIFAR100 & 87.45 & +0.299 & 0.032 * \\
    \hline
    \rowcolor{near!50}
    TIN & 89.95 & +0.330 & 0.002 ** \\
    \hline
    \rowcolor{near!50}
    Near-OOD Avg. & 88.70 & +0.315 & <0.001 *** \\
    \hline
    \hline
    \rowcolor{far!50}
    \multicolumn{4}{|c|}{Far-OOD} \\
    \hline
    \rowcolor{far!50}
    MNIST & 94.85 & +0.584 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    SVHN & 94.34 & +0.048 & 0.259 \\
    \hline
    \rowcolor{far!50}
    Texture & 94.81 & +0.077 & 0.211 \\
    \hline
    \rowcolor{far!50}
    Places365 & 89.47 & +0.410 & <0.001 *** \\
    \hline
    \rowcolor{far!50}
    Far-OOD Avg. & 93.37 & +0.280 & 0.001 ** \\
    \hline
    \end{tabular}
    \caption[T-test for GradCAMNorm salvim on CIFAR10]{Results of performing a t-test on the \ac{auroc} means of GradCAMNorm salvim against \ac{mls} and \ac{msp}, showing the mean \ac{auroc} over 10 runs on CIFAR10, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
    \label{table:cifar10_gradcam_Norm_salvim_ttest}
\end{center}
\setlength\tabcolsep{6pt}
\end{table}
...
Overall, the results of integrating \ac{xai} saliencies directly into the already developed \ac{vim} \ac{ood} detector shows potential, but is not consistently better on both datasets.

\section{Summary}

In this chapter, I have presented the results from inspecting saliency aggregation over the validation dataset, as well results from the three different \ac{xai} based \ac{ood} detection algorithms I introduced in chapter \ref{chapter:methodology}. As we have seen, the results are very promising, and show that there is definite potential for using \ac{xai} methods for \ac{ood} detection. \ac{gbp}Norm+Logits is a particularly interesting example, performing far above the baseline on ImageNet200 and almost at the level of \ac{sota} methods on CIFAR10 (amongst methods which do not require retraining).

\chapter{Discussion} \label{chapter:discussion}

This chapter will discuss the findings of chapter \ref{chapter:experiments} in more depth. As we have now seen, \ac{xai} methods can indeed perform \ac{ood} detection at a level competitive with \ac{ood} detection baselines, and can in some cases even perform close to \ac{sota} models amongst methods which do not retrain the underlying classifying network.
...
%
%
% In this chapter, I will conduct a thorough analysis of the explanations generated by different \ac{xai} methods. The focus will be on the possible differences between explanations made on \ac{id} and \ac{ood} data points. With this in mind, I will compare and contrast the different methods, attempting to identify possible candidate methods for use in \ac{ood} detection.
%
...
%
% To investigate the potential for \ac{xai} methods to aid in separating \ac{id} from \ac{ood} data, I introduce a simple collection of datasets, which form an \ac{ood} detection problem. This collection consists of an \ac{id}, Near-\ac{ood} and Far-\ac{ood} dataset. The \ac{id} dataset is ImageWoof \cite{imagewoof}, a subset of ImageNet which contains ten different breeds of dogs which are to be classified. For Near-\ac{ood} data points, the obvious choice would be pictures of dogs which are of different breeds than the 10 breeds used in ImageWoof. Thus, I use the Stanford Dog Dataset \cite{stanforddogs}, and simply remove any classes which overlap with the 10 from ImageWoof. Given that dogs from two different breeds can have many similarities in terms of size, colour, texture and body and face structure, the Stanford Dog Dataset represents a very small semantic shift and consequently, a difficult \ac{ood} detection task.
%
...
% \end{figure}
%
% With this collection, I train a ResNet-50 model on the \ac{id} dataset and subsequently run inference on all three datasets, generating explanations for all data points.
%
% \section{Results}
%
