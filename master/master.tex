...
As explained previously, the $\delta$ must be set by the 
...
\subsubsection{Maximum Logit + Saliency Aggregate}

In 2024, \cite{combood} introduced \texttt{COMBOOD}. This \ac{ood} detector combines the \ac{ood} detection scores of two different distance metrics; Mahalanobis distance and nearest neighbour distance. They do this by summing confidence scores computed from the log distributions of these two metrics. \texttt{COMBOOD} achieves \ac{sota} performance, and is (at the time of writing) by far the best performing method on ImageNet200 in the OpenOOD benchmark. Based on these results, I propose a similar method for combining \ac{xai}-based and traditional \ac{ood} detection metrics.

For this method, the \ac{ood} score is a sum of the maximum logit and a saliency aggregate. However, due to the fact that both the logit and saliency values can be of any arbitrary magnitude, if we want them to be equally important, we must normalize them before summing. Thus, can sum the Z-scores of each metric instead. This ensures that the values of the maximum logit and the saliency aggregate are distributed in about the same way. To calculate the Z-scores, we can simply subtract the mean and divide by the standard deviation over an entire \ac{id} validation dataset, for each metric. Thus, we calculate the mean and standard deviations of the max logit over an \ac{id} validation set $\mu_{\text{MLS}}^{id}$ and $\sigma_{\text{MLS}}^{id}$, as well as the mean and standard deviation of the aggregate of saliencies $\mu_{\text{Agg}}^{id}$ and $\sigma_{\text{Agg}}^{id}$. In addition, we must calculate the mean value of the aggregation metric over a validation \ac{ood} dataset, as we do not know whether a given aggregation is higher or lower for \ac{id} data. We denote this value as $\mu_{\text{Agg}}^{ood}$. We now have the necessary values required to define this method mathematically:

% A simple and intuitive way to combine the information retrieved from \ac{xai} saliency methods and traditional \ac{ood} detection algorithms is to simply add them together. If we have two metrics, both of which are higher for \ac{id} data than \ac{ood} data, then a sum of these two could increase the separation further. This intuition has been utilized by \cite{combood}. Their method combines two distance metrics by a simple unweighted addition, and achieves \ac{sota} performance on ImageNet200. Thus, a similar approach with saliency and traditional \ac{ood} detection metrics could prove effective. As a baseline, I propose using the maximum logit and an aggregate of saliency. Let us define this method mathematically:

As in the previous section, we assume we have a model $f: \bm{x} \to \R^C$, an \ac{xai} saliency mapping method $s: (f, \bm{x}) \to \R^{H_s \times W_s}$, and an aggregation function $A: \bm{x} \rightarrow \R$. The \ac{ood} detector then has the following form, given a threshold $\delta$:

{\large
\begin{align}
    g(\bm{x}; s, A, \delta)=\begin{cases} 
    \text{in } &  \text{sign}(\mu_{\text{Agg}}^{id} - \mu_{\text{Agg}}^{ood}) \cdot \frac{A(s(\bm{x}, f)) - \mu_{\text{Agg}}^{id}}{\sigma_{\text{Agg}}^{id}} + \frac{\max_i S(\bm{x}) - \mu_{\text{MLS}}^{id}}{\sigma_{\text{MLS}}^{id}} \ge \delta \\[10pt]
    \text{out} &  \text{sign}(\mu_{\text{Agg}}^{id} - \mu_{\text{Agg}}^{ood}) \cdot \frac{A(s(\bm{x}, f)) - \mu_{\text{Agg}}^{id}}{\sigma_{\text{Agg}}^{id}} + \frac{\max_i S(\bm{x}) - \mu_{\text{MLS}}^{id}}{\sigma_{\text{MLS}}^{id}} < \delta \\
   \end{cases}
\label{eq:aggregate}
\end{align}
}
...
As is common when applying \ac{lime} to images, I have used segmentations of the input image to reduce the dimensionality of the input that is fed into the \ac{lime} algorithm. As superpixel segmentation algorithms such as \ac{slic} are CPU-bound and thus introduce a considerable computational overhead (which is problematic when tens of thousands of samples are to be evaluated), I have chosen to stick to a simple rectangular segmentation instead. Each image is simply split into $N \times N$ equally sized rectangular regions, with $N$ being set to 4 during the experiments unless otherwise specified.

\subsection{Occlusion}

Like with \ac{lime}, I have used a rectangular segmentation approach, with each image being split into $N \times N$ regions. $N$ is set to 4 during the experiments. This ensures that accurate comparisons between \ac{lime} and occlusion can be made.
...
Python is the most popular programming language for data science and \ac{ml} research \cite{nguyen2019machine}, and as such it is my language of choice as well. Below is a table of the key libraries I have used.
...
This section contains the final tests conducted on the test set, using the \ac{xai} methods that performed the best on the validation set, as reported in the previous section. Based on those results, a reasonable choice will be to use \ac{gradcam}, \ac{gbp} and integrated gradients as the \ac{xai} saliency generators, with the vector norm as the aggregation. This section will be divided into three parts, corresponding to the three proposed \ac{ood} detection methods introduced in the methodology: Saliency Aggregation, Saliency Aggregation plus Logits and SaliencyVIM. Each of these sections will detail the performance of the corresponding method on the test set, and statistically compare the results to the baseline methods.

\subsection{Results for Saliency Aggregation} \label{section:salagg_results}

Saliency Aggregation, as described in \ref{section:saliencyagg_method}, is the first \ac{ood} detection method that will be tested. As we have seen from the validation set, saliency aggregation on its own can sometimes compete with the baselines on Near-\ac{ood} and 
