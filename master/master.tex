...
OpenOOD includes 11 different benchmarks across Anomaly Detection, Open Set Recognition and \ac{ood} detection, three fields which are very closely related. Of these, 6 benchmarks are used to test methods for \ac{ood} detection, which are the ones I will be concerned with. Each benchmark is defined by an ID dataset, with 6 or more corresponding \ac{ood} datasets, separated into Near-\ac{ood} and Far-\ac{ood}. For each benchmark, \ac{auroc}, \ac{aupr} and \ac{fpr95} is reported over all \ac{ood} datasets, and Near-\ac{ood} \ac{auroc} is used to rank methods against each other.
...
\subsubsection{Aggregate of Saliency} \label{section:saliencyagg_method}
...
As in the previous section, we assume we have a model $f: \bm{x} \to \R^C$, an \ac{xai} saliency mapping method $s: (f, \bm{x}) \to \R^{H_s \times W_s}$, and an aggregation function $A: \bm{x} \rightarrow \R$. Given that the logit and the aggregate of saliencies can have wildly different magnitudes, we should also normalize them so that they have about the same magnitude. 

% Normalization involves subtracting the mean and dividing by the standard deviation of the whole dataset. However, when doing \ac{ood} detection (or any form of binary classification) we are only concerned about 

For this, we can divide each metric by the mean of that metric on an \ac{id} validation set. Thus, we further assume we have mean of the max logit $\mu_{\text{MLS}}$ and a mean of the aggregate of saliencies $\mu_{\text{Agg}}$. The \ac{ood} detector then has the following form, given a threshold $\delta$:

{\large
\begin{align}
    g(\bm{x}; s, A, \delta)=\begin{cases} 
        \text{in } & \frac{A(s(\bm{x}, f))}{\mu_{\text{Agg}}} + \frac{\max_i S(\bm{x})}{\mu_{\text{MLS}}} \ge \delta \\
        \text{out} & \frac{A(s(\bm{x}, f))}{\mu_{\text{Agg}}} + \frac{\max_i S(\bm{x})}{\mu_{\text{MLS}}} < \delta 
   \end{cases}
\label{eq:aggregate}
\end{align}
}
