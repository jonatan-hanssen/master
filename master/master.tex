...
These two problems lead to the fields of machine learning Explainable Artificial Intelligence (XAI), and Out-of-Distribution (OOD) detection. Both fields have seen increased interest in recent years, as machine learning is being used more and more for tasks which can have a high impact on individuals (e.g. cancer detection or automated driving). This thesis will attempt to combine the two fields, by using methods inspired by XAI to improve OOD detection.

\section{Problem Statement}

As explained in the previous segment, OOD detection is a developing and complicated field, which has become more important in recent years as machine learning is being used for higher impact tasks, such as disease detection.  Finding novel methods which improve a model's ability to detect when input is OOD is important to increase the robustness of machine learning models as they are used in these real-world scenarios. The problem statement is thus as follows:
