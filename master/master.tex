...
\begin{figure}[H]
    ...
    \node [above=0.1cm of 4] {Near-OOD: CIFAR100};
    \node [above=0.1cm of 7] {Near-OOD: TinyImageNet};
    \node [above=0.1cm of 10] {Far-OOD: MNIST};
    \node [above=0.1cm of 13] {Far-OOD: SVHN};
    \node [above=0.1cm of 16] {Far-OOD: Texture};
    \node [above=0.1cm of 19] {Far-OOD: Places365};
    ...
    \caption[CIFAR10 dataset example images]{Figure showing three example images from CIFAR10 and from the corresponding OOD datasets}
...
\begin{figure}[H]
    ...
    \node [above=0.1cm of 4] {Near-OOD: SSB-Hard};
    \node [above=0.1cm of 7] {Near-OOD: NINCO};
    \node [above=0.1cm of 10] {Far-OOD: iNaturalist};
    \node [above=0.1cm of 13] {Far-OOD: Texture};
    \node [above=0.1cm of 16] {Far-OOD: OpenImage-O};
    ...
    \caption[ImageNet200 dataset example images]{Figure showing three example images from ImageNet200 and from the corresponding OOD datasets}
...
\section{Hypothetical OOD Detection Problem: Dog Breed Classification Neural Network}

Complicated concepts are often more easily understood when the reader is given examples. Thus, to aid the reader in their understanding of my methodology, I will introduce a simple hypothetical OOD detection task, which will be used as the context whenever I illustrate a method with an example. This OOD detection task consists of a Neural Network which is trained on a dataset of ten different dog breeds, henceforth known as {\it Dogs10}. The task of the network is to correctly classify an image of a dog into one of the ten classes. In addition to the ID dataset Dogs10, I imagine a Near-OOD dataset called {\it DifferentDogs} which contains dogs of different breeds than the 10 the model is trained on. Finally I imagine a Far-OOD dataset called {\it NoDogs}, which contain images where no dog is present.

\section{Data Analysis of Explanations}

As part of my effort to integrate XAI methods into OOD detection, I plan to perform detailed data analysis on the properties of explanations generated by different XAI methods. The core of this analysis will be comparisons between explanations generated on ID and OOD data.

\subsection{Data Generation}

As explained previously, I limit myself to the study of saliency mapping XAI methods. This means that each explanation is a two-dimensional grid of real values, which denote the importance of each region, as determined by the XAI method used. By iterating over the entire ID dataset, as well as all associated OOD datasets, we thus have explanations corresponding to all data points, both ID and OOD.

Specifically, given a trained model $f$, an XAI saliency mapping method $s$ and a data set $D$ containing images $\{ \mathbf{x} \mid \mathbf{x} \in \R^{c, h, w} \}$, I will generate a dataset of explanations $D_{sal}$ in the following way:
...
Like the Mean Saliency OOD Detector, we imagine a model trained on dogs, which is fed two images, one which contains a dog and one which does not. In this case, we do not care about the magnitudes, but instead only the spread of the values in relation to each other, and thus the heatmap is normalized. As we can see, the heatmap is more spread out in the explanation where there is no dog present than for the image with a dog. By calculating a suitable metric, such as the Gini coefficient, entropy or another measure of spread, we can get a single number which represents how spread out the heatmap is, regardless of its magnitude. Using these values, we can define a threshold which is below most ID data and above most OOD data, allowing us to separate these distributions.
