...
%
% Finally, we inspect the results of the t-tests for \ac{gbp}Norm+Logit, which are very impressive. Here, we see substantial improvements over both baselines of several percentage points on both Near- and Far-\ac{ood}. Furthermore, these results are pretty close to the \ac{sota} amongst methods which do not require retraining of the network, with \texttt{COMBOOD} being the leader at 91.13/94.65 Near-\ac{ood}/Far-\ac{ood} performance.
...
\begin{tabular}{ |c|c|c|c| }
    ...
    \hline
    \end{tabular}
    \caption[Wilcoxon signed-rank test for salvim on Imagenet]{Results of performing a Wilcoxon signed-rank test on the \ac{auroc} means of against \ac{vim}, showing the mean \ac{auroc} over 10 runs on Imagenet, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
...
\begin{tabular}{ |c|c|c|c| }
    ...
    \hline
    \end{tabular}
    \caption[Wilcoxon signed-rank test for salvim on CIFAR100]{Results of performing a Wilcoxon signed-rank test on the \ac{auroc} means of against \ac{vim}, showing the mean \ac{auroc} over 10 runs on CIFAR100, the difference in means compared to the baselines, and the corresponding p-values. Each p-value is appended a significance code which follows the \texttt{R}-standard.}
